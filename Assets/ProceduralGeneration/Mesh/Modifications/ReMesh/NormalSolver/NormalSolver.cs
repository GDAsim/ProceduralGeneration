...
{
    /// <summary>
    /// implemented vertex splitting for already smoothed meshes. For instance, if a mesh is already smoothed with a 180° angle, 
	/// the solver won't bring smoothing back to 0 when requested.
	/// "https://gist.github.com/runevision/6fd7cc8d841245a53df5d09ccf6b47ff?permalink_comment_id=3285882#gistcomment-3285882"
    /// </summary>
    ...
    public static void UnweldVertices(Mesh mesh)
    {
        ...
        Vector2[] unweldedUV = new Vector2[unweldedVertices.Length];

        for (int i = 0; i < triangles.Length; i++)
        {
            unweldedVertices[i] = vertices[triangles[i]];
            unweldedTriangles[i] = i;
            unweldedUV[i] = uvs[triangles[i]];
        }
        mesh.vertices = unweldedVertices;
        mesh.triangles = unweldedTriangles;
        mesh.uv = unweldedUV;
